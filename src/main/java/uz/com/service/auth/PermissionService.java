...
import uz.com.dto.auth.PermissionAccessDto;
...
public class PermissionService implements IPermissionService {
    ...
    @Autowired
    public PermissionService(PermissionDao permissionDao, PermissionMapper permissionMapper, GenericMapper genericMapper, BaseUtils utils, IGeneralRepository generalRepository) {
...
//    @PreAuthorize("hasPermission(null, T(uz.com.enums.Permissions).PERMISSION_READ)")
    public ResponseEntity<DataDto<List<PermissionDto>>> getAll(PermissionCriteria criteria) {
        ...
        Stream<Permission> stream = permissionDao.listStream(criteria);
        return new ResponseEntity<>(new DataDto<>(permissionMapper.toDto(stream.collect(Collectors.toList())), total), HttpStatus.OK);
    }

    @Override
    public ResponseEntity<DataDto<List<PermissionAccessDto>>> getAllToAccess(PermissionCriteria criteria) {
        Long total = permissionDao.total(criteria);
        Stream<Permission> stream = permissionDao.listStream(criteria);
        List<PermissionAccessDto> permissionAccessDtos = permissionMapper.toAccessDtos(stream.collect(Collectors.toList()));
        permissionAccessDtos.forEach(permissionAccessDto -> {
            permissionAccessDto.setRoles(generalRepository.hasRoleExistPermission(permissionAccessDto.getId()));
        });
        return new ResponseEntity<>(new DataDto<>(permissionAccessDtos, total), HttpStatus.OK);
    }

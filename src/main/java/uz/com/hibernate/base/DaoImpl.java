...
public class DaoImpl<T extends _Entity, C extends GenericCriteria> implements Dao<T, C> {
    ...
    @Override
    public Stream<T> listStream(C criteria) {
    ...
    @Override
    public Stream findStream(String query, Map<String, ?> params) {
    ...
    @Override
    public List find(String query, Map<String, ?> params) {
        Query queryObject = getSession().createQuery(query);
        if (params != null) {
            for (Map.Entry<String, ?> entry : params.entrySet()) {
                queryObject.setParameter(entry.getKey(), entry.getValue());
    ...
    @Override
    public Stream findIntervalStream(String query, Map<String, ?> params, Integer page, Integer perPage) {
        Query queryObject = getSession().createQuery(query);
        if (params != null) {
            for (Map.Entry<String, ?> entry : params.entrySet()) {
                queryObject.setParameter(entry.getKey(), entry.getValue());
        ...
        return getResultsStream(queryObject, page, perPage);
    }

    @Override
    public List findInterval(String query, Map<String, ?> params, Integer page, Integer perPage) {
        Query queryObject = getSession().createQuery(query);
        if (params != null) {
            for (Map.Entry<String, ?> entry : params.entrySet()) {
                queryObject.setParameter(entry.getKey(), entry.getValue());
    ...
    @Override
    public Stream findNativeIntervalStream(String query, Map<String, ?> params, Integer page, Integer perPage) {
    ...
    @Override
    public List findNativeInterval(String query, Map<String, ?> params, Integer page, Integer perPage) {
        Query queryObject = getSession().createNativeQuery(query);
        if (params != null) {
            for (Map.Entry<String, ?> entry : params.entrySet()) {
                queryObject.setParameter(entry.getKey(), entry.getValue());
    ...
    protected Stream getResultsStream(Query query, Integer page, Integer perPage) {
    ...
    protected List getResults(Query query, Integer page, Integer perPage) {
        if ((page == null || perPage == null) || (page < 0 || perPage <= 0)) {
            return query.list();
        } else {
            return query.setFirstResult(page * perPage).setMaxResults(perPage).list();
        }
    }
